...
export async function sendOrderConfirmationEmail(to: string, d: { firstName: string; orderNumber: string; items: Array<{ name: string; quantity: number; color: string; technique?: string }>; totalPrice: string; estimatedDelivery: string }) {
...
export async function sendCartAbandonedEmail(to: string, d: { firstName: string; items: Array<{ name: string; price: string }>; totalPrice: string; cartUrl: string }) {
  const list = d.items.map(i => `<li style="margin-bottom:4px">${i.name} — ${i.price}</li>`).join("");
  return sendEmail({ to, subject: "Tienes productos esperándote 🎁", html: T(`<h2 style="font-size:24px;font-weight:800">¿Olvidaste algo?</h2><p style="color:#666">Hola ${d.firstName}, dejaste productos en tu carrito:</p><ul style="color:#444;line-height:2;padding-left:20px">${list}</ul><p style="font-weight:700;font-size:18px;color:#DE0121">Total: ${d.totalPrice}</p><p style="text-align:center"><a href="${d.cartUrl}" class="btn">Completar pedido</a></p>`) });
}
...
export async function notifyAdminProofRejected(d: { orderNumber: string; clientName: string; productName: string; reason: string }) {
  return sendEmail({ to: ADMIN_EMAIL, subject: `⚠️ Proof rechazado: ${d.orderNumber}`, html: T(`<h2>Proof rechazado</h2><div class="ab" style="background:#FEE2E2"><p><strong>Pedido:</strong> ${d.orderNumber}</p><p><strong>Cliente:</strong> ${d.clientName}</p><p><strong>Producto:</strong> ${d.productName}</p><p><strong>Motivo:</strong> ${d.reason}</p></div>`) });
}
