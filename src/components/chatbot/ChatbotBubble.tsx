...
export function ChatbotBubble() {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    if (wizardState.step >= 0 && wizardState.step < 3) {
      ...
      // Update locally keeping the role: user visible
      ...
      if (nextStep < 3) {
          ...
          try {
             ...
             setWizardState(w => ({ ...w, step: 5 })); // Terminado, el chat sigue libre
          }
      }
      return;
    }
      ...
      {isOpen && (
        <div className="fixed bottom-[180px] right-6 md:bottom-[170px] md:right-8 z-50 w-[90vw] md:w-[380px] h-[75vh] md:h-[600px] max-h-[800px] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-gray-100 animate-in fade-in slide-in-from-bottom-4">
          ...
          <div className="flex-1 overflow-y-auto p-4 bg-surface-50 scroll-smooth">
            {messages.length === 0 && wizardState.step === -1 && (
              <div className="text-center text-gray-500 mt-10">
                ...
                <p className="text-xs mt-1 opacity-70">Pregúntame por mochilas o usa nuestra herramienta guiada.</p>
                ...
                <div className="mt-8 flex flex-col gap-2 w-full px-4">
                  ...
                  <button type="button" onClick={() => { sendMessage({ role: 'user', content: 'Busco mochilas o material de oficina' }) }} className="bg-white border border-gray-200 text-gray-700 py-2 px-4 rounded-xl text-sm shadow-sm hover:bg-gray-50 flex items-center justify-center gap-2">
