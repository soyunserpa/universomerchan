...
const buildQuizSteps = (t: (key: string) => string): Question[] => [
  {
    id: "q1",
    field: "volume",
    title: t("modal.q1.title"),
    subtitle: t("modal.q1.subtitle"),
    type: "choice",
    choices: [
      { value: "S", label: t("modal.q1.choices.s") },
      { value: "M", label: t("modal.q1.choices.m") },
      { value: "L", label: t("modal.q1.choices.l") },
      { value: "XL", label: t("modal.q1.choices.xl") }
    ]
  },
  {
    id: "q2",
    field: "objective",
    title: t("modal.q2.title"),
    type: "choice",
    choices: [
      { value: "Regalos para Empleados", label: t("modal.q2.choices.employees") },
      { value: "Ferias y Eventos", label: t("modal.q2.choices.events") },
      { value: "Regalos VIP", label: t("modal.q2.choices.vip") },
      { value: "Campaña Promocional", label: t("modal.q2.choices.promo") }
    ]
  },
  {
    id: "q3",
    field: "budget",
    title: t("modal.q3.title"),
    type: "choice",
    choices: [
      { value: "Super Económico", label: t("modal.q3.choices.low") },
      { value: "Económico a Medio", label: t("modal.q3.choices.mid") },
      { value: "Medio a Premium", label: t("modal.q3.choices.high") },
      { value: "Luxury", label: t("modal.q3.choices.vip") }
    ]
  },
  {
    id: "q4",
    field: "industry",
    title: t("modal.q4.title"),
    type: "choice",
    choices: [
      { value: "Tecnología", label: t("modal.q4.choices.tech") },
      { value: "Hosteleria", label: t("modal.q4.choices.hospitality") },
      { value: "Salud", label: t("modal.q4.choices.health") },
      { value: "Educacion", label: t("modal.q4.choices.education") },
      { value: "Construccion", label: t("modal.q4.choices.construction") },
      { value: "Belleza", label: t("modal.q4.choices.beauty") },
      { value: "Agencia", label: t("modal.q4.choices.agency") },
      { value: "Otro", label: t("modal.q4.choices.other") }
    ]
  },
  {
    id: "q_lead",
    field: "email",
    title: t("modal.qLead.title"),
    subtitle: t("modal.qLead.subtitle"),
    type: "lead_gate",
    placeholder: ""
  }
];
      ...
      <div className="fixed inset-0 z-[100] bg-white flex flex-col overflow-y-auto w-full h-full animate-in fade-in zoom-in-95 duration-500">
        ...
        <div className="flex-1 max-w-4xl w-full mx-auto p-4 sm:p-12 mt-4 sm:mt-12">
          ...
          <div className="mt-12 bg-gray-50 rounded-2xl border border-gray-100 p-8 text-center mb-24">
            <h3 className="text-2xl font-bold text-gray-900 mb-4">{t("modal.requestFinalQuote")}</h3>
            <p className="text-gray-600 mb-8 max-w-md mx-auto">{recommendedPack.closing}</p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <a href={`mailto:pedidos@universomerchan.com?subject=${encodeURIComponent(t("modal.mailSubject", { title: recommendedPack.title }))}`} className="inline-flex w-full sm:w-auto items-center justify-center px-8 py-4 bg-brand-red text-white flex gap-2 rounded-xl font-bold hover:bg-red-700 hover:-translate-y-1 transition-all shadow-lg">
                {t("modal.contactSales")}
              </a>
              <a href="https://wa.me/34600000000" target="_blank" rel="noopener noreferrer" className="inline-flex w-full sm:w-auto items-center justify-center px-8 py-4 bg-[#25D366] text-white flex gap-2 rounded-xl font-bold hover:bg-[#1ebe57] hover:-translate-y-1 transition-all shadow-lg">
        ...
        <div className="flex-1 overflow-y-auto p-6 sm:p-12 flex flex-col justify-center">
          ...
          {currentStep.type === "lead_gate" && (
            <form onSubmit={handleTextSubmit} className="flex flex-col gap-4">
              ...
              <p className="text-center text-xs text-gray-400 mt-2">{t("modal.spamDisclaimer")}</p>
            </form>
          )}
        </div>
      </div>
