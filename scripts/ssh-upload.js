...
conn.on('ready', () => {
    conn.sftp((err, sftp) => {
        if (err) throw err;
        const readStream = fs.createReadStream('scripts/test-midocean-order.ts');
        const writeStream = sftp.createWriteStream('/var/www/universomerchan/scripts/test-midocean-order.ts');
